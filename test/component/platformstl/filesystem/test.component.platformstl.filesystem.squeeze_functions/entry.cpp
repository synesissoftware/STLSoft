 ...
 *
 * Purpose: Unit-tests for `platformstl::squeeze_path`.
 *
...
int main(int argc, char *argv[])
{
    ...
    if (XTESTS_START_RUNNER("test.component.platformstl.filesystem.squeeze_functions", verbosity))
    {
...
static void TEST_NULL_BUFFER()
{
    // stem only
    {
        ...
        {
            size_t const cch = platformstl::path_squeeze(input, static_cast<char*>(NULL), 4);

            TEST_INT_EQ(7, cch);
        }

        {
            size_t const cch = platformstl::path_squeeze(input, static_cast<char*>(NULL), 10);

            TEST_INT_EQ(7, cch);
        }

        {
            size_t const cch = platformstl::path_squeeze(input, static_cast<char*>(NULL), 11);

            TEST_INT_EQ(7, cch);
        }

        {
            size_t const cch = platformstl::path_squeeze(input, static_cast<char*>(NULL), 12);
...
static void TEST_SUFFICIENT_SPACE()
{
    // stem only
    {
        ...
        {
            char            buff[101];
            size_t const    cch = platformstl::path_squeeze(input, &buff[0], STLSOFT_NUM_ELEMENTS(buff));
    ...
    // basename
    {
        ...
        {
            char            buff[28];
            size_t const    cch = (buff[25] = buff[26] = buff[27] = '#', platformstl::path_squeeze(input, &buff[0], STLSOFT_NUM_ELEMENTS(buff)));
        ...
        {
            char            buff[29];
            size_t const    cch = (buff[25] = buff[26] = buff[27] = buff[28] = '#', platformstl::path_squeeze(input, &buff[0], STLSOFT_NUM_ELEMENTS(buff)));
    ...
    // full path
    {
        ...
        {
            char            buff[23];
            size_t const    cch = (buff[22] = '#', platformstl::path_squeeze(input, &buff[0], STLSOFT_NUM_ELEMENTS(buff)));
    ...
    // full path (short basename)
    {
        ...
        {
            char            buff[23];
            size_t const    cch = (buff[14] = buff[15] = '#', platformstl::path_squeeze(input, &buff[0], STLSOFT_NUM_ELEMENTS(buff)));
...
static void TEST_INSUFFICIENT_SPACE()
{
    // stem only
    {
...

        {
            ...
            size_t const    cch = platformstl::path_squeeze(input, &buff[0], STLSOFT_NUM_ELEMENTS(buff) - 1);
